...
CREATE TABLE `biz_user_preference` (
  ...
  `team_code` varchar(32) DEFAULT NULL COMMENT '组队码 (优先级最高)',
  ...
  `personal_hygiene` tinyint DEFAULT '3' COMMENT '个人卫生评分: 1-5分',
  ...
  `ac_temp` tinyint DEFAULT '26' COMMENT '空调温度: 18-30',
  ...
  `game_type_mmo` tinyint DEFAULT '0' COMMENT '玩MMO(剑三/魔兽): 0-否 1-是',
  `game_type_mobile` tinyint DEFAULT '0' COMMENT '玩手游(王者/原神): 0-否 1-是',
  ...
  `mbti_e_i` char(1) DEFAULT 'E' COMMENT 'E/I维度',
  `mbti_result` varchar(4) DEFAULT NULL COMMENT 'MBTI结果(如INTJ)',
  `social_battery` tinyint DEFAULT '3' COMMENT '社交意愿: 1-社恐(别理我), 5-社牛',
  `share_items` tinyint DEFAULT '1' COMMENT '物品共享: 0-皆不可, 1-部分可借(纸/伞), 2-随意用',
  `bring_guest` tinyint DEFAULT '0' COMMENT '带人回寝: 0-绝不, 1-偶尔同性, 2-经常, 3-带异性',
  `visitors` tinyint DEFAULT '0' COMMENT '访客频率: 同bring_guest',
  `relationship_status` tinyint DEFAULT '0' COMMENT '恋爱状态: 0-单身, 1-恋爱中(可能会煲电话粥)',
